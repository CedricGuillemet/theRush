...
void main() 
{
	...
	vec4 ShadowCoordPostW = coord/coord.w;//vec4(pos.xy, pos.z, pos.w) / coord.w;
	
	for (int i=0;i<4;i++)
	{
		vec2 decal = vec2(poisson[i].x*rotRandom.x - poisson[i].y*rotRandom.y, poisson[i].x*rotRandom.y + poisson[i].y*rotRandom.x) * vec2(1./4096., 1./1024.) * 2.0;
