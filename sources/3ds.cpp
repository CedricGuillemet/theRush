...
bool CLoad3DS::Import3DS(t3DModel *pModel, const char *strFileName)
{
	...
	if(!m_FilePointer) 
	{
        LOGDEBUG("Unable to find the file" << strFileName << "!\n");
	...
	{
        LOGDEBUG("Unable to load PRIMARY chuck from file:" << strFileName << "!\n");
...
void CLoad3DS::ProcessNextChunk(t3DModel *pModel, tChunk *pPreviousChunk)
{
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		case VERSION:							// This holds the version of the file
            ...
            // Read version    
            m_CurrentChunk->bytesRead += fread(&version, 1, 2, m_FilePointer);
			...
			if (version > 0x03)
            {
                LOGDEBUG("This 3DS file is over version 3 so it may load incorrectly\n")
		...
		case OBJECTINFO:						// This holds the version of the mesh
			...
			// and OBJECT chunks.  From here on we start reading in the material and object info.
                ...
                LOGDEBUG("CHUNK object info" << std::endl);
            ...
            /*{
                char cbuffer[128];
                unsigned int rl = fread(cbuffer, 1, 128, m_FilePointer);
                cbuffer[rl] = '\0';
                printf(string(cbuffer).c_str());
            }*/
		...
		case MATERIAL:							// This holds the material information
			...
			ProcessNextMaterialChunk(pModel, m_CurrentChunk);
                ...
                LOGDEBUG("CHUNK material " << pModel->pMaterials[pModel->numOfMaterials-1].strName << std::endl);
			break;
                
                
        case OBJECT_SMOOTH_GROUP:
            LOGDEBUG("CHUNK smooth group");
            break;
		...
		case OBJECT:							// This holds the name of the object being read
			...
			// holds the name of the object.
                ...
                LOGDEBUG("CHUNK object" << std::endl);
        ...
        default: 
            LOGDEBUG("Main default: "<< std::hex << m_CurrentChunk->ID << std::endl);
...
void CLoad3DS::ProcessNextObjectChunk(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
        ...
        case OBJECT_SMOOTH_GROUP:
			/*
            LOGDEBUG("CHUNK smooth group" << std::endl);
            ReadSmoothGroups(pModel, pObject, m_CurrentChunk);
            m_CurrentChunk->bytesRead += fread(buffer, 1, m_CurrentChunk->length - m_CurrentChunk->bytesRead, m_FilePointer);
			*/
            break;
        ...
        case OBJECT_TRANSFORM:
            LOGDEBUG("CHUNK object transform" << std::endl);
            ReadMeshTransform(pModel, pObject, m_CurrentChunk);
            m_CurrentChunk->bytesRead += fread(buffer, 1, m_CurrentChunk->length - m_CurrentChunk->bytesRead, m_FilePointer);
            break;
		...
		default: 
            LOGDEBUG("Next default: "<< std::hex << m_CurrentChunk->ID << std::endl);
...
{
	static char strMaterial[255] = {0};			// This is used to hold the objects material name
	static int buffer[50000] = {0};				// This is used to read past unwanted data
    ...
    tMaterialInfo* materialToApply = NULL;
    int materialID = -1;
	// Go through all of the textures
	for(int i = 0; i < pModel->numOfMaterials; i++)
	{
        //LOGDEBUG("Material visited " << pModel->pMaterials[i].strName << std::endl);
        
		// If the material we just read in matches the current texture name
		if(strcmp(strMaterial, pModel->pMaterials[i].strName) == 0)
		{
            materialToApply = &pModel->pMaterials[i];
            materialID = i;
		}
	}
    //LOGDEBUG("ModelNumMaterial = " << pModel->numOfMaterials << std::endl);
    
    if(materialToApply==NULL)
    {
        //LOGDEBUG("Material " << strMaterial << "NOT found!" << std::endl);
    }
    
    
    int nbFaceMaterial = 0;
    fread(&nbFaceMaterial, 1, 2, m_FilePointer);
    //LOGDEBUG("nbFaceMaterial " << nbFaceMaterial << std::endl);
    pPreviousChunk->bytesRead+=2;
    
    
    if(nbFaceMaterial && materialToApply!=NULL)
    {
        unsigned short* faceMat = new unsigned short[nbFaceMaterial];
        fread(faceMat, 1*nbFaceMaterial, 2, m_FilePointer);
        pPreviousChunk->bytesRead+=2*nbFaceMaterial;
    
        if(pObject->pFaces!=NULL)
        {
            for(int fm=0; fm<nbFaceMaterial; ++fm)
            {
                pObject->pFaces[faceMat[fm]].mat = materialToApply;
            }
        }
        else
        {
            LOGDEBUG("Reading mesh material faces without faces ids!!!" << std::endl);
        }
    ...
    /////////////////////////////////////////////////////// I keep that here but it is bullshit (works only for sinlge material mesh)
	...
	// Yes though, we could have just passed in the model and not the object too.
		// If the material we just read in matches the current texture name
    if(materialToApply != 0)
    {
        //LOGDEBUG(strMaterial << " found " << std::endl);
        // Set the material ID to the current index 'i' and stop checking
        //LOGDEBUG("pObject->materialID = " << pObject->materialID << ", i=" << i << std::endl);
        pObject->materialID = materialID;
        //LOGDEBUG("pObject->materialID = " << pObject->materialID << ", i=" << i << std::endl);
        LOGDEBUG(pObject->strName << " uses material " << pModel->pMaterials[materialID].strName << std::endl);
        
        // Now that we found the material, check if it's a texture map.
        // If the strFile has a string length of 1 and over it's a texture
        if(strlen(pModel->pMaterials[materialID].strFile) > 0) {
...
void CLoad3DS::ReadSmoothGroups(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
    unsigned int nentries = (pPreviousChunk->length - 6) / 4;
	
    if(pObject->numOfFaces!=nentries)
    {
        pObject->pFaceSmoothGroup = NULL;
        LOGDEBUG("Wrong smooth group entries (numFace="<<pObject->numOfFaces<<", numSmoothGroups="<<nentries<<".");
    }
    else
    {
        pObject->pFaceSmoothGroup = new unsigned int [pObject->numOfFaces];
        
        // skip remaining chunk data
        pPreviousChunk->bytesRead += fread(pObject->pFaceSmoothGroup, pObject->numOfFaces, 4, m_FilePointer);
        
        LOGDEBUG("Face smooth group loaded.");
    }
    /*
...
void CLoad3DS::ReadMeshTransform(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
    pPreviousChunk->bytesRead += fread(pObject->x, 1, 3*sizeof(float), m_FilePointer);
    pPreviousChunk->bytesRead += fread(pObject->y, 1, 3*sizeof(float), m_FilePointer);
    pPreviousChunk->bytesRead += fread(pObject->z, 1, 3*sizeof(float), m_FilePointer);
    pPreviousChunk->bytesRead += fread(pObject->o, 1, 3*sizeof(float), m_FilePointer);
    
    //swap<float>(pObject->x[1], pObject->x[2]);
    //swap<float>(pObject->y[1], pObject->y[2]);
    //swap<float>(pObject->z[1], pObject->z[2]);
    
    pObject->o[1]*=-1;
    std::swap(pObject->o[1], pObject->o[2]);
    
    LOGDEBUG("X: x="<< pObject->x[0] <<" y="<< pObject->x[1] <<" z="<< pObject->x[2] << std::endl);
    LOGDEBUG("Y: x="<< pObject->y[0] <<" y="<< pObject->y[1] <<" z="<< pObject->y[2] << std::endl);
    LOGDEBUG("Z: x="<< pObject->z[0] <<" y="<< pObject->z[1] <<" z="<< pObject->z[2] << std::endl);
    LOGDEBUG("O: x="<< pObject->o[0] <<" y="<< pObject->o[1] <<" z="<< pObject->o[2] << std::endl);
}
